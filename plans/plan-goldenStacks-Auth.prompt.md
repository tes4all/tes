 ...
 **Identity & Auth (`stacks/zitadel` & `stacks/authentik`)**
    *   **Integration**: Configured to use the external `postgres-ha` stack (via overlay network) and `edge-router` (via labels).
    *   **Stateless**: App containers optimized for scaling.
